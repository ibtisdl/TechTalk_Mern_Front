...
export const getAllUsers = async (
  ...
  try {
    ...
    const { data, headers } = await axios.get(
      `http://localhost:5000/api/users?searchKeyword=${searchKeyword}&page=${page}&limit=${limit}`,
      config
    );
    return { data, headers };
  } catch (error) {
...
export const deleteUser = async ({ slug, token }) => {
  try {
    ...
    const { data } = await axios.delete(`http://localhost:5000/api/users/${slug}`, config);
    return data;
  } catch (error) {
    if (error.response && error.response.data.message)
      throw new Error(error.response.data.message);
